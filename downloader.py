...
def requestNewDownloadUrl(downloadUrl, node, path, master_albums_list, oauth_data):
    ...
    if response.status_code == 202:
        ...
        logger.error('Request to queue failed for (uri, path): (%s, %s) ' % (downloadUrl, path))
...
def initiateDownload(node, path, sleep_queue, master_albums_list, api_key_params, oauth_data):
    ...
    if not response.status_code == 200:
        logger.error('Error getting node for dir: ' + path)
        return
    ...
    if 'Download' in responseJson:
        ...
        for downloadJson in responseJson['Download']:
            ...
            logger.info("triggering download for %s" % fileName)
...
def loadConfigData():
    try:
        ...
        logger.error('Could not open config.json. Please ensure it exists')
        sys.exit(1)
...
if __name__ == '__main__':
    ...
    while master_albums_list:
        ...
        logger.info('Waiting for all files to be processed... remaining: %d' % len(master_albums_list))
        logger.info("albums: %s" % master_albums_list)
        sleep(10)
    logger.info('Done!')
